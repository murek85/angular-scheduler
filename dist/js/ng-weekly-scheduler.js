...
angular.module('weeklyScheduler')
  ...
  .directive('weeklyScheduler', ['$parse', 'weeklySchedulerTimeService', '$log', function ($parse, timeService, $log) {
    ...
    function config(schedules, options) {
      ...
      var minDate = (!angular.isUndefinedOrNull(options.minDate) ? options.minDate : (schedules ? schedules.reduce(function (minDate, slot) {
        return timeService.compare(slot.start, 'isBefore', minDate);
      }, now) : now)).startOf('week');

      // Calculate max date of all scheduled events
      var maxDate = (!angular.isUndefinedOrNull(options.maxDate) ? options.maxDate : (schedules ? schedules.reduce(function (maxDate, slot) {
        return timeService.compare(slot.end, 'isAfter', maxDate);
      }, now) : now)).endOf('week');
      // }, now) : now).clone().endOf('week');
...
angular.module('weeklySchedulerI18N')
  .provider('weeklySchedulerLocaleService', ['tmhDynamicLocaleProvider', function (tmhDynamicLocaleProvider) {
    ...
    var defaultConfig = {
      doys: {'de-de': 4, 'en-gb': 4, 'en-us': 6, 'fr-fr': 4, 'pl-pl': 6},
      lang: {
        ...
        'pl-pl': {month: 'Miesiąc', weekNb: 'Tydzień #', addNew: 'Dodaj'}
      }
    };
...
angular.module('ng-weekly-scheduler/views/multi-slider.html', []).run(['$templateCache', function($templateCache) {
  $templateCache.put('ng-weekly-scheduler/views/multi-slider.html',
    '<weekly-slot class=slot ng-class="{disable: item.editable === false}" ng-repeat="schedule in item.schedules" ng-model=schedule ng-model-options="{ updateOn: \'default blur\', debounce: { \'default\': 500, \'blur\': 0 } }"></weekly-slot>');
}]);
...
angular.module('ng-weekly-scheduler/views/weekly-scheduler.html', []).run(['$templateCache', function($templateCache) {
  $templateCache.put('ng-weekly-scheduler/views/weekly-scheduler.html',
    '<div class=labels-area-container><div class=label-area><div class="srow text">{{schedulerCtrl.config.labels.month || \'Month\'}}</div><div class="srow text">{{schedulerCtrl.config.labels.weekNb || \'Week number\'}}</div><div class=schedule-animate ng-repeat="item in schedulerCtrl.items" inject></div></div></div><div class=schedule-area-container><div class=schedule-area><div class="srow timestamps"><monthly-grid class=grid-container></monthly-grid></div><div class="srow timestamps"><weekly-grid class=grid-container></weekly-grid></div><div class="srow schedule-animate" style="background: white" ng-repeat="item in schedulerCtrl.items"><weekly-grid class="grid-container striped" no-text></weekly-grid><multi-slider index={{$index}}></multi-slider></div></div></div>');
}]);
